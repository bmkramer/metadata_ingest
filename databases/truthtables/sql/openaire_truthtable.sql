...
WITH RORS_ARRAY AS (
  ...
  FROM (
    ...
    FROM `sos-datasources.openaire.organization_20250912`,
...
PROJECTS AS (
  ...
  FROM (
    ...
    FROM `sos-datasources.openaire.project_20250912`,
...
SOURCES AS (
   ...
   FROM `sos-datasources.openaire.publication_20250912` as publications
...
UNION ALL
   ...
   FROM `sos-datasources.openaire.dataset_20250912` as publications
...
UNION ALL
   ...
   FROM `sos-datasources.openaire.software_20250912` as publications
...
UNION ALL
   ...
   FROM `sos-datasources.openaire.otherresearchproduct_20250912` as publications
),
...
LEFT JOIN (SELECT
  ...
  FROM SOURCES as publications
  LEFT JOIN (SELECT * FROM `sos-datasources.openaire.relation_product_organization_20250912` WHERE relType.type = "affiliation") as relations
...
--- note: all citation relations between results and results are unique -- not confirmed for 20250912
--- Cites n=2,334,772,909, IsCitedBy n=432,203,400 ?
...
LEFT JOIN (SELECT
  target as id,
  COUNTIF(relType.name = "Cites") as citations,
  FROM `sos-datasources.openaire.relation_product_product_cites_20250912`
  GROUP BY id) as citation
ON publications.id = citation.id
...
--- note: tables other than publication used to have pid.value and pid.scheme inversed... keep check
DOIS AS (
