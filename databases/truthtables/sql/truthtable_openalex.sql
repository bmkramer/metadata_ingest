...
SELECT * FROM `subugoe-collaborative.openalex_walden.works` --- 20260202
...
SELECT
    ...
    id as source_id,
    type,
    is_xpac, --- to enable filtering on in-/exclusion of expansion pack data 
    ...
    -- Abstracts
    --- abstract_inverted_index not included in current dataset, use has_abstract (Boolean)
    ---CASE
   ...
   -- Fields and topics
    ...
    CASE
        WHEN (SELECT COUNT(1) FROM UNNEST(funders) AS funder WHERE funder.ror is not null) > 0 THEN TRUE
        ELSE FALSE
    END as has_funders_id_ror,
    (SELECT COUNT(1) FROM UNNEST(funders) AS funder WHERE funder.ror is not null) as count_funders_id_ror,    
